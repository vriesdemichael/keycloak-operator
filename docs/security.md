...
|------|---------------|------------|---------------|
| **Operator Token** | Internal token, bootstrap token (dev mode) | Token for simple single-tenant setups | ❌ NOT the same as operational token |
| **Admission Token** | Bootstrap token (production), initialization token | One-time token to set up a namespace | ❌ NOT for ongoing operations |
| **Operational Token** | Auto-rotating token, namespace token | Long-lived token with automatic rotation | ❌ NOT manually managed |
| **Realm Token** | Realm authorization token | Per-realm token for client management | ❌ NOT used for realm creation |
...
**Characteristics:**
- ✅ Simple: One token for everything
- ✅ Fast setup: No token management needed
- ❌ No multi-tenancy: All teams share one token
- ❌ No automatic rotation: Must rotate manually
- ❌ Not production-ready: Token compromise affects all realms
...
**Characteristics:**
- ✅ Multi-tenant: Each namespace has independent token
- ✅ Auto-rotation: Tokens rotate automatically
- ✅ Namespace isolation: Token compromise limited to one namespace
- ✅ Production-ready: Security best practices
- ⚠️ More complex: Requires initial admission token setup

### Key Concepts

| Concept | Explanation | Example |
|---------|-------------|---------|
| **Token Discovery** | Operator searches namespace for operational token using labels instead of explicit reference. Simplifies realm creation after bootstrap. | Realm spec omits `authorizationSecretRef` → operator finds `team-a-operator-token` via labels |
...
| **Bootstrap** | Process of converting admission token to operational token when first realm is created. | First realm uses admission token → operator generates operational token |
...
- ✅ Scales to unlimited teams (no ClusterRole updates)
- ✅ Self-service (platform team shares secret, team proceeds)
- ✅ GitOps-native (secrets are standard K8s resources)
