...
class TestMonitoringResources:
    ...
    def test_monitoring_disabled_by_default(self, chart_path):
        ...
        assert len(service_monitors) == 0, (
            "ServiceMonitor should not exist when monitoring disabled"
        )

        # Check no PrometheusRule
        prometheus_rules = [doc for doc in docs if doc.get("kind") == "PrometheusRule"]
        assert len(prometheus_rules) == 0, (
            "PrometheusRule should not exist when monitoring disabled"
        )
        ...
        assert len(dashboards) == 0, (
            "Dashboard ConfigMap should not exist when monitoring disabled"
        )
    ...
    def test_prometheusrule_created_when_enabled(self, chart_path):
        ...
        assert expected_alerts.issubset(alert_names), (
            f"Missing expected alerts: {expected_alerts - alert_names}"
        )
    ...
    def test_prometheusrule_alert_structure(self, chart_path):
        ...
        for rule in rules:
            ...
            assert "severity" in rule["labels"], (
                f"Rule missing 'severity' label: {rule}"
            )
            assert "component" in rule["labels"], (
                f"Rule missing 'component' label: {rule}"
            )
            assert rule["labels"]["component"] == "keycloak-operator"

            # Check annotations
            assert "summary" in rule["annotations"], (
                f"Rule missing 'summary' annotation: {rule}"
            )
            assert "description" in rule["annotations"], (
                f"Rule missing 'description' annotation: {rule}"
            )
            assert "runbook_url" in rule["annotations"], (
                f"Rule missing 'runbook_url' annotation: {rule}"
            )
    ...
    def test_grafana_dashboard_created_when_enabled(self, chart_path):
        ...
        assert expected_panels.issubset(panel_titles), (
            f"Missing expected panels: {expected_panels - panel_titles}"
        )
            ...
            if "targets" in panel:
                for target in panel["targets"]:
                    assert "expr" in target, (
                        f"Panel '{panel['title']}' missing Prometheus query"
                    )
                    expr = target["expr"]
    ...
    def test_all_monitoring_resources_together(self, chart_path):
        ...
        dashboards = [
            ...
            and "dashboard" in d.get("metadata", {}).get("name", "")
        ]
