...
async def shared_operator(
    ...
    try:
        ...
        # Teardown: Retrieve coverage data before cleanup
    logger.info("Shared operator teardown starting...")

    # Retrieve coverage data if enabled
    if coverage_enabled:
        logger.info("Coverage enabled - retrieving coverage data from operator pod...")
        try:
            # Find operator pod
            pods = await k8s_core_v1.list_namespaced_pod(
                namespace=operator_namespace,
            ...
            if pods.items:
                ...
                try:
                    ...
                    if "NO_FILES" not in resp:
                        coverage_files = [f for f in resp.strip().split("\n") if f]
                        logger.info(f"Found {len(coverage_files)} coverage file(s)")

                        # Copy each coverage file
                        for coverage_file in coverage_files:
                        ...
                        logger.info(f"✓ Retrieved coverage data to {coverage_dir}")
                    else:
                        logger.warning("No coverage files found in operator pod")
                except Exception as copy_error:
                    logger.warning(f"Failed to copy coverage files: {copy_error}")
            else:
                logger.warning("No operator pod found for coverage retrieval")
        except Exception as e:
            logger.warning(f"Failed to retrieve coverage data: {e}")
            logger.warning("Coverage data may be incomplete")
